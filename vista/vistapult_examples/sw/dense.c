...
#define DENSE_BASE    ((volatile unsigned int *) 0x80000000)

#define TB_WRITE(ADDR, DATA)          *(ADDR) = (DATA); 
#define TB_READ(ADDR, DATA)           DATA = *(ADDR);

#define DNS_GO(X)                     TB_WRITE(DENSE_BASE +  0, X)  
#define DNS_GO_READY(X)               TB_READ (DENSE_BASE +  1, X)
#define DNS_DONE(X)                   TB_WRITE(DENSE_BASE +  2, X)
#define DNS_DONE_VALID(X)             TB_READ (DENSE_BASE +  3, X)
#define DNS_IMAGE_OFFSET(X)           TB_WRITE(DENSE_BASE +  4, X)
#define DNS_WEIGHT_OFFSET(X)          TB_WRITE(DENSE_BASE +  5, X)
#define DNS_BIAS_OFFSET(X)            TB_WRITE(DENSE_BASE +  6, X)
#define DNS_OUTPUT_OFFSET(X)          TB_WRITE(DENSE_BASE +  7, X)
#define DNS_NUM_UNITS(X)              TB_WRITE(DENSE_BASE +  8, X)
#define DNS_UNIT_COUNT(X)             TB_WRITE(DENSE_BASE +  9, X)
#define DNS_OUTPUT_ELEMENTS(X)        TB_WRITE(DENSE_BASE + 10, X)
#define DNS_RELU(X)                   TB_WRITE(DENSE_BASE + 11, X)
#define DNS_BIAS(X)                   TB_WRITE(DENSE_BASE + 12, X)
