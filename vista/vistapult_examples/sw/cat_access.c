...
unsigned int float_to_cat(float f)
{
    ...
    integer_portion = (int) f;
    fractional_portion = f - (float) integer_portion;

    if ((f < 0.0) && (fractional_portion != 0.0)) {
       integer_portion--;
       fractional_portion += 1.0;
    }

    integer_bits = integer_portion << FRACTIONAL_BITS;
    fractional_bits = fractional_portion * (1 << FRACTIONAL_BITS);
    fractional_bits &= ((1 << FRACTIONAL_BITS) - 1);

    bit_pattern = integer_bits + fractional_bits;

#if WORD_SIZE < 32
    bit_pattern &= ((1 << WORD_SIZE) - 1);
#endif

    return bit_pattern;  
}

float cat_to_float(int cat_value)
{
    float value;

#if WORD_SIZE < 32
    unsigned int sign_bit;
    unsigned int extension_mask = 0xFFFFFFFF << WORD_SIZE;
    float value;

    sign_bit = cat_value >> (WORD_SIZE - 1);

    if (sign_bit) {
       cat_value |= extension_mask;
    }
#endif
    value = ((float) cat_value)/((float)(1 << FRACTIONAL_BITS));
...
void CLASS_PREFIX set_cat_value(cat_memory_type *memory, int offset, float value)
{
    unsigned long    bit_pattern = float_to_cat(value);
    unsigned int     array_word;
    unsigned int     index = offset / (WORD_SIZE/32);
    unsigned long    shift = (offset % (WORD_SIZE/32)) * WORD_SIZE;
#if WORD_SIZE==32
    unsigned int     mask = 0xFFFFFFFF;
#else
    unsigned int     mask = ((1 << WORD_SIZE) - 1) << shift;
#endif
    unsigned int     inverse_mask = ~mask;

    array_word = memory[offset/STRIDE];
    array_word = array_word & inverse_mask; 
    array_word = array_word | ((bit_pattern << shift) & mask);
...
#ifdef CHECK_WRITES
       set_value = get_cat_value(memory, offset+i);
       if (set_value != source[i]) {
          printf("Expected: %f got: %f bit_pattern: %08x \n", source[i], set_value, 0xFFFFFFFF);
       }
#endif
    }
}
