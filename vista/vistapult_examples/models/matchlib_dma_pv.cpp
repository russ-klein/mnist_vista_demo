...
void matchlib_dma_pv::cb_write_source(unsigned int newValue) {
  send_to_set_source(offsetof(dma_address_map, ar_addr), newValue);
}
...
void matchlib_dma_pv::cb_write_destination(unsigned int newValue) {
  send_to_set_source(offsetof(dma_address_map, aw_addr), newValue);
}
...
void matchlib_dma_pv::cb_write_size(unsigned int newValue) {
  send_to_set_source(offsetof(dma_address_map, len), newValue-1);
}
...
void matchlib_dma_pv::cb_write_go(unsigned int newValue) {
  send_to_set_source(offsetof(dma_address_map, start), newValue);
  done = 0;
}
...
void matchlib_dma_pv::slave_r_process() {
  ...
  while(1) {
    ...
    do {
      int data;
      master_read((int) ar.addr, &data, 1);
      // printf("read: %d from: %8x \n", data, ar.addr);
      r.data = data;
...
void matchlib_dma_pv::slave_w_process() {
  ...
  while(1) {
    ...
    do {
      w = tb_w_slave.w.Pop();
      int data = w.data;
      master_write((int) aw.addr, &data, 1);
      // printf("writing %d at %8x \n", data, aw.addr);
    } while (tb_w_slave.next_multi_write(aw));
    wait();  // nedded ensure that response happens after all the writes
...
void matchlib_dma_pv::send_to_set_source(unsigned int addr, unsigned int data) 
{
  transaction_addr = addr;
  transaction_data = data;
  send_transaction.notify();
}
...
void matchlib_dma_pv::set_source()
{
  ...
  while(1) {
    wait(send_transaction);
    tb_w_master.single_write((int) transaction_addr, (int) transaction_data);
    wait();
  }
}
